...
For example: if the RQ is written in terms of *proportions*, then the hypotheses, CI and so forth should also be written in terms of *proportions*.
...
`r if( knitr::is_html_output() ) {'<!--'}`
   ...
   c. Either of the above.
`r if( knitr::is_html_output() ) {'-->'}`
4. Suppose the sample odds ratio is found to be $0.51$. 
   Explain very clearly what this odds ratio **means** in context.
5. In the study, the odds that someone received a mosquito bite when a *wax* candle was being used was\ $2.17$.
   What are the odds that someone in the study received a mosquito bite when a *citronella* candle was used? 
6. Suppose the CI is found to be $0.23$ to $1.1$. 
   Explain **very clearly** what this CI **means** in context.
