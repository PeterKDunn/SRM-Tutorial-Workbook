...
8. The $z$-score is 0.385 from [Table B.3](https://bookdown.org/pkaldunn/SRM-Textbook/ZTablesOnlineBackwards.html), remembering that the area to the *left* would be 0.650 (draw a diagram!). 
...
5. $\text{s.e.}(\hat{p}) = \sqrt{0.304455 \times (1-0.304455)/404} = \sqrt{0.00052416} = 0.022894$, or about 0.023.  
   A definition can be found in the [Glossary](https://bookdown.org/pkaldunn/SRM-Textbook/Glossary.html). 
...
8. One way of writing communicating: “The population proportion of patients surviving after BVM treatment has a 95% chance of lying between 26% and 35%.”
   This is not strictly correct, but acceptable and very, very commonly used (as explained in [the textbook](https://bookdown.org/pkaldunn/SRM-Textbook/CIInterpretation.html)).
...
12. Either (or both) of these must be true:
    ...
    and/or 
    (ii) the sample size is large enough so that the sample means have a normal distribution, so about larger than 25.
13. Since $n < 25$, we need to assume the population of differences has a normal distribution.
    A stem-and-leaf plot suggests this is not unreasonable, so the sample means quite possibly have an approx.\ normal distribution:
...
1. This is just one box o fmatches (one *observation*), but the claim is about the *population mean*.
...
Use the [68--95--99.7 rule](https://bookdown.org/pkaldunn/SRM-Textbook/identifying-outliers.html#def:EmpiricalRule)
to [*approximate* the $P$-values](https://bookdown.org/pkaldunn/SRM-Textbook/Pvalues.html#ApproxP):
...
### Thoughts on Sect. \@ref(HTOneProportionA) {.unlisted .unnumbered}



According to [Statistica.com](https://www.statista.com/statistics/431375/individuals-who-wear-contact-lenses-in-selected-european-countries/), 30% of individuals in Sweden wore contact lenses in 2020 (the highest percentages in Europe).
...
\]
(make sure to use $p$ not $\hat{p}$ in the standard error calculation!), and so
...
1. The third one. 
   The first is about samples. 
   The second is about means.
...
5. The answers:
    ...
    a. No answer. But a line is fine for **Plot 1**, **Plot 2**, **Plot 3** (but very weak!) and **Plot 6**,  but not for **Plot 4** and **Plot 6**. 
