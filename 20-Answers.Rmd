...
### Answers for Sect.\ \@ref(UnderstandGraphsPizza) {.unlisted .unnumbered}
...
4. Two-way table or stacked/side-by-side barchart, or table; two qualitative variables ($30 mins$ of physical activity or not; vigorous physical 
...
1. To display the relationship between the age of athletes, and the times taken for each athlete to complete the `beep test': *Scatterplot*
   Boxplot if 'Whether aged under\ $25$'.
...
1. $\bar{x} = 16.02\ms$.
2. $s = 7.145\ms$; $\text{s.e.}(\bar{x}) = s/\sqrt{n} = 7.145/\sqrt{44} =  1.077\ms$. 
...
3.The CI is from $13.85$ to $18.19\ms$.
4. $95$% CI for population mean guess: $13.85$ to $18.19\ms$.
...
1. $s = 7.145\ms$; $\text{s.e.}(\bar{x}) = s/\sqrt{n} = 7.145/\sqrt{44} =  1.077\ms$.  
...
2. The CI is from $13.85$ to $18.19\ms$.
3.  Strong to moderate evidence exists in the sample (one sample $t = 2.714$; $\text{df} = 43$; two-tailed $P = 0.010$) that the population mean guess (mean: $16.02\ms$; standard deviation: $7.145\ms$) of the students is not $13.1\ms$ ($95$%\ CI for population mean guess: $13.85$ to $18.19\ms$).
...
6. $\bar{d} = 24.9$; $s_d=13.03372\ms$.
7. $\text{s.e.}(\bar{d}) = s_d/\sqrt{n} = 13.03372/\sqrt{10} = 4.121623\ms$.
8. This is the standard deviation of the sample mean difference, a measurement of how precisely the sample mean difference measures the population mean difference.
9. Almost impossible.
...
10. Normal; mean $\mu$, std deviation is the standard error of $4.121$.
11. $24.9\pm (2\times 4.121623)$, or $24.9\pm 8.243246$, or from $16.65675$ to $33.143245\ms$.
12. $H_0$: $\mu_d = 0$, differences defined as 'with' minus 'without'. 
    $H_1$: $\mu_d > 0$ the way I defined differences.  
13. $t = 6.041$ and since $t$ is very large, expect $P$ to be very small.
14. The differences have just been defined in the opposite directions.
   (Notice the $P$-values are the same.)
15. Very strong evidence exists in the sample (paired $t = 6.041$; one-tailed $P$ less than $0.0005$) that the population mean 2MWT are higher after receiving the implant compared to without the implant (mean difference: $24.9\ms$ higher after receiving the implant; standard deviation: $13.034\ms$; $95$%\ CI from $16.66$ to $33.14\ms$).
16. The population of differences has a normal distribution, and/or $n > 25$ or so.
17. Since $n < 25$, must assume the population of differences has a normal distribution.
...
13. The the *sample means* have a normal distribution.
    Since both sample sizes are greater than\ $24$, this is true.
...
8. $H_0$: $\mu_d = 0$, differences defined as 'after' minus 'before'. 
   $H_1$: $\mu_d > 0$ the way I defined the differences.  
...
6. $t = 0.9016$, so $P$-value large: no evidence of difference. 
7. Since $n < 25s$, we require that the *differences* in the population have a normal distribution.
8. The stem-and-leaf plot of the *sample* doesn't suggest the *population* is non-normal; probably OK.
...
1. Observational.
2. Relational.
3. Two completely different groups of pizzas (stores) are compared.
4. $-0.774$ to $-0.560$ inches (differences are *Dominos* less *Eagle Boys*).
5. 
6. From output, $t = -14.6$ and $P < 0.001$: very strong evidence of a difference.
7. Since the sample sizes are large (both $125$), we *do not require that the populations have normal distributions*.
8. The sample sizes are large ($n = 125$ in each), so we don't need the populations to be normally distributed; **we don't need the histogram**.
9. Probably true. 
   Amount of topping on the pizza? 
   Which *tastes* better? 
   Whether the samples were randomly selected or not?
   Are sizes before of after cooking?
...
9. Probably not; OR close to\ $1$, and CI includes\ $1$.
...
if( knitr::is_latex_output() ) {
   kable(MatSumm,
        ...
        longtable = FALSE,
        booktabs = TRUE,
        caption = "Nunmerical summary table for maturation data.",
        align = c("r", "r", "r", "r"))   %>%
   kable_styling(full_width = FALSE, 
                 font_size = 8) %>%
  row_spec(0, bold = TRUE) # Columns headings in bold
...
1. $\chi^2 = 8.74$ and $P = 0.003$; strong evidence of a difference.
...
4. $b_0 = 1.531$ and $b_1 = 0.311$.
5. $\hat{y} = (0.311 \times 20) + 1.531 = 7.751$.
6. Slope of\ $0.311$: for each extra year of age, the stem diameter increases by about\ $0.311\mms$ on average.
   Intercept of\ $1.531$: for stems of zero years of age, the average stem diameter is about $1.531\mms$, which is silly. 
...
**1.** P:\ school age children with ambulatory cerebral palsy; 
       O:\ In this case, the hand-writing legibility;
       C:\ Between scores made when using standard and special furniture; 
       I:\ The furniture configurations imposed.
