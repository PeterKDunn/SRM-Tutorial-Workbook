...
$H_0$: $p = 0.2$ and $H_1$: $p < 0.2$.
Here, $n = 202$ and $\hat{p} = 15/202 = 0.074257$, with $p = 1/5 = 0.2$.
Then,

\[
   \text{s.e.}(\hat{p}) = \sqrt{\frac{0.2 \times 0.8}{202}} = 0.0281439.
\]
(Make sure to use $p$ not $\hat{p}$ in the standard error calculation!)
Then

\[
   z = \frac{0.074257 - 0.2}{0.0281439} = -4.47. 
\]
This is quite large and negative, and the one-tailed $P$-value is very small ($P < 0.001$).
There is very strong evidence that students do worse than guessing on this question.

(The CI: Need to use $\hat{p}$ in the standard error calculation, so $\text{s.e.}(\hat{p}) = 0.018448$ and so the approximate 95% CI is from 0.037 to 0.111.)
