...
* Numerical summaries for **qualitative** data (e.g., odds, proportions).
* Graphical summaries for comparing **qualitative** data.
* Numerical summaries for comparing **qualitative** data (e.g., odds ratios, difference in proportions).
:::
...
"Odds") )} else{'\\null'}`
\greyboxlines{1}
1. What might be an **appropriate** way to numerically describe the average  Respiratory Event Index (REI) of participants?
`r if( knitr::is_html_output() ) {webexercises::longmcq( c(
"Standard deviation", 
answer = "Median",
"Odds",
...
\greyboxlines{1}
1. In the sample of $n = 60$, what **percentage** of individuals are females?
`r if( knitr::is_html_output() ) {webexercises::longmcq( c(
"0.45", 
answer = "45%",
"0.55",
...
"0.82") )} else {'\\null'}`
\greyboxlines{1}
1. In the sample of $n = 60$, what are the **odds** that an individual is female?
`r if( knitr::is_html_output() ) {webexercises::longmcq( c(
"0.45", 
"45%",
"0.55",
...
\greyboxlines{2}
1. What proportion of patients had their stroke symptoms missed? \tightlist
\greyboxlines{2}
1. Sketch a side-by-side or stacked bar chart to display the data.
\greyboxlines{4}
1. Of the *male* patients, what *proportion* had their stroke symptoms missed by the dispatcher?
`r if( knitr::is_html_output() ) {webexercises::longmcq( c(
answer = "0.391", 
"0.609",
"0.524",
"0.333") )} else {'\\null'}`
\greyboxlines{2}
1. Of the *female* patients, what *proportion*  had their stroke symptoms missed by the dispatcher?
`r if( knitr::is_html_output() ) {webexercises::longmcq( c(
"0.713",
"0.476",
answer = "0.287", 
...
"1.24") )} else {'\\null'}`
\greyboxlines{2}
1. What is the *difference between the proportions* of stroke symptoms being missed by the dispatcher, comparing *males* to *females*?
`r if( knitr::is_html_output() ) {webexercises::longmcq( c(
"0.391",
"1.6",
answer = "0.104",
...
answer = "1.6") )} else {'\\null'}`
\greyboxlines{2}
1. Construct a numerical summary table (see Table\ \@ref(tab:StrokeSymptomsSummary)).
...
if( knitr::is_latex_output() ) {
  ...
  StrokeSumm[5, 2] <- cell_spec( StrokeSumm[5, 2],
    ...
    kable(StrokeSumm,
          ...
          caption = "Summary table: emergency dispatchers and missing stroke symptoms.") %>%
...
if( knitr::is_html_output() ) {
  ...
  kable(StrokeSumm[ c(1, 3, 5), ],
        ...
        escape = FALSE,
        longtable = FALSE,
        caption = "Summary table: emergency dispatchers and missing stroke symptoms. Enter PROPORTIONS and ODDS to TWO decimal places.",
        booktabs = TRUE)  %>%
    add_header_above( c(" " = 1,
                        "Proportion" = 1,
                        "Odds" = 1,
