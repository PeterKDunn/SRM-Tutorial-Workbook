...
1. A *confidence interval* is used to find an interval that probably contains the *sample* value. True or false?  
...
```{block2, type="fold"}
1. A CI is an interval which probably contains the **population** value. We already know the **sample** value... we don't need an interval for it.
1. Confidence intervals can be any percentage (though 90%, 95% and 99% CIs are the most common, and 95% easily the most common of all).
1. A *standard deviation* is a measure of variation in the *data*; a *standard error* is a measure of how much a sample summary value varies from sample to sample.
...
They purchased a package containing twenty boxes of matches,
and Jake counted the number of matches in one of the 20 boxes.
...
2. Then, 
   they counted the number of matches in *each* of the twenty boxes.  
   
   Claire found that the mean number of matches per box
   was 44.9 matches,
   and the standard deviation was $0.11$.  
...
## **Optional**: Sample size calculations {#RoomWidthCISampleSize}
...
if( knitr::is_html_output() ) {
  WaveTab[15, 3] <- inTableWebex(webex::fitb(num=TRUE, tol=0.001, width=6, answer=WaveTab[15,3]))
  WaveTab[16, 3] <- inTableWebex(webex::fitb(num=TRUE, tol=0.001, width=6, answer=WaveTab[16,3]))
  WaveTab[17, 3] <- inTableWebex(webex::fitb(num=TRUE, tol=0.001, width=6, answer=WaveTab[17,3]))
  WaveTab[18, 3] <- inTableWebex(webex::fitb(num=TRUE, tol=0.001, width=6, answer=WaveTab[18,3]))
} else {
