# Teaching Week 9: tutorial  {#Lecture8}
...
```{block2, type="fold"}
1. if we take the *coeliac* mean minus the *non-coeliac* mean, we would have $8.39 - 8.17 = 0.22$, which is what is given in the table.
1. The CI is for the *difference* in the DMFT. The value is **not** about the *number* of teeth; it is about the *difference* in the number of teeth between coeliacs and non-coeliacs. A negative value is fine if we understand what the CI is estimating.
...
*(This question continues Question \@ref(HM:Maturing).)*
...
*(Answers are available in Sect. \@ref(Lecture8Answers))*
