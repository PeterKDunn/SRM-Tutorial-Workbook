...
1. Develop your own RQ to answer the `r readr::parse_character( c("café"), locale = locale(encoding = "UTF-8"))` owner's question.
1. For your RQ, describe P, O, C and I (where relevant).
1. Make notes about *how* the `r readr::parse_character( c("café"), locale = locale(encoding = "UTF-8"))` owner could answer the question.
...
> &nbsp;
>
> 'Everyone knows that Earl Grey tea relaxes students...' said Jane, looking upwards then closing her eyes.   
> &nbsp;
>
...
2. Identify an *outcome* that Nim can measure to answer the research question.
   Justify your answer.
3. Which of these *comparisons* do you think Nim should use?
   Why?
...
The *Janka hardness*^[The force required to embed an $11.28$\ mm steel ball into wood to a depth of half the diameter of the ball.] of bamboo flooring provided by Bamboo Flooring Australia Pty Ltd was measured by the Queensland Department of Primary Industries [@data:Gerber:BambooFlooring].
Five floorboards were taken, and two hardness measurements were taken on each board  (units probably kilonewtons; Table\ \@ref(tab:JankaBoards)).
...
```{r Crossword-Week1-Terminology, echo=FALSE, fig.cap="A crossword to complete", fig.align="center", out.width='75%', eval=is_latex_output()}
...
\begin{enumerate}\tightlist
  \item[\textbf{4.}] 
  The first step in the research process is to \verb|____| the question (3)  
  \item[\textbf{5.}]
  RQs in which groups are compared are called \verb|____| (10)  
  \item[\textbf{6.}]
  `Conceptual' and `Operational' are types of \verb|____| (11)  
  \item[\textbf{8.}]
  RQs whose main purpose in \emph{not} making a decision are \verb|____| RQs (10)  
  \item[\textbf{9.}]
  The type of research where data are \emph{not} summarised using numerical methods (11)  
  \item[\textbf{11.}]
  Research questions are answered using \verb|____| (that is, the data) (8)  
  \item[\textbf{13.}]
  Data are recorded from the \verb|____| of observation (5)  
  \item[\textbf{14.}]
  What we do with the data to answer the RQ (7)  
  \item[\textbf{15.}]
...
\begin{itemize}\tightlist
  ...
  \item[\textbf{2.}]
  The type of research where data are summarised using numerical methods (12)  
  \item[\textbf{3.}]
